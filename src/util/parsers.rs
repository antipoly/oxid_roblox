...
where
  D: Deserializer<'de>,
{
  Deserialize::deserialize(deserializer).and_then(|date_string: &str| date_string.parse::<DateTime<Utc>>().map_err(Error::custom))
}
...
pub fn parse_optional_base_universe<'de, D>(deserializer: D) -> Result<Option<BaseUniverse>, D::Error>
where
  D: Deserializer<'de>,
{
